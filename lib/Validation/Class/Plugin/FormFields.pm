...
{
  $Validation::Class::Plugin::FormFields::VERSION = '0.0.5';
}
...
it IS NOT. Validation::Class::Plugin::FormFields is not an HTML form and/or
...
So why why render fields individually and not the entire form (fields collection).
Form generation is pretty evil IMO, whereas the generating of HTML elements is
less evil and definately alot more rational. Full-blown form generation locks
...
Render elements indivually, obviously we have to generate some output or this
plugin would be a ridiculous waste of space, however .. the generated HTML is
sensible and consistent to allow easy CSS styling and JavaScript manipulations.

Ever tried to add an HTML5 <button>Click Me</button> or Horizontal Rule <hr/> to
your form that was generated by that form rendering package you used to use ...?
Don't render forms with code, Take back control of your controls.
...
The HTML form field elements are rendered via TT template (template-toolkit).
...
Once copied and modified to your liking, specify the CWD in your validation
